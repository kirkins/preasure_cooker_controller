...
int warmupTemp       = 24;// set to 96
int defaultTemp      = 28;// set to 118
...
void mainHeat() {
  ...
  targetTemp = defaultTemp + (((analogRead(heatDial)-1023)*(-1))/1023) * 20;
  currentTemp = getTemp();
  heatingPID.Compute();
...
double getHeatDial() {
  double tempAdjustment = (((analogRead(heatDial)-1023)*(-1))/1023) * 20;
